...
def do_initialize(): #-> (mvnc.Device, mvnc.Graph):
    ...
    graph_filename = filefolder + '/model2.graph'
...
def do_inference(graph, image_filename, number_results): #-> (List[str], List[numpy.float16]) :
    ...
    graph.LoadTensor(image_for_inference, None)
...
def show_inference_results(image_filename, infer_labels, infer_probabilities):# -> None:
...
def main():
    ...
    for index in range(0, len(image_name_list)):
        infer_labels, infer_probabilities = do_inference(graph, image_name_list[index], 1)
